...
echo
echo "2が不要な場合は、あとでスマートフォンのRokidアプリからWi-Fi ONを削除してください。"
echo "場所: ホーム → ツールボックス → メガネのアプリ管理"
echo
...
if ! adb -s "$SERIAL" install -r "$APK" </dev/null; then
    ...
    echo "    → スマートフォンのRokidアプリで「ホーム」→「ツールボックス」→"
    echo "      「メガネのアプリ管理」を開き、Wi-Fi ONを削除してからもう一度お試しください。"
